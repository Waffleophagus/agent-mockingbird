...
export default tool({
  description:
    "Read a safe slice of canonical markdown memory files by path and line window.",
  args: {
    path: tool.schema
      .string()
      .min(1)
      .describe("Memory path such as MEMORY.md or memory/2026-02-17.md"),
    from: tool.schema
      .number()
      .int()
      .min(1)
  ...
  async execute(args: { path: string; from?: number; lines?: number }) {
    const payload = await postJson("/api/mockingbird/memory/read", {
      path: args.path,
      from: args.from,
      lines: args.lines,
    });
