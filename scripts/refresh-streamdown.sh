...
node - "$ROOT_DIR/package.json" "$ROOT_DIR/apps/mobile/package.json" "$react_native_version" "$core_version" "$code_native_version" <<'EOF'
const fs = require("node:fs");

const [rootPath, mobilePath, reactNativeVersion, coreVersion, codeNativeVersion] =
...
rm -rf "$ROOT_DIR/apps/mobile/node_modules/@streamdown" "$ROOT_DIR/node_modules/@streamdown"
