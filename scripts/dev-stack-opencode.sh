...
concurrently -k -n executor,server -c magenta,green \
  ...
  "bash -lc 'set -euo pipefail; deadline=\$((SECONDS + ${STARTUP_TIMEOUT_SECONDS})); echo \"[stack] waiting for Executor at ${EXECUTOR_HEALTH_URL}\"; until curl -fsS \"${EXECUTOR_HEALTH_URL}\" >/dev/null 2>&1; do if (( SECONDS >= deadline )); then echo \"[stack] timed out waiting for Executor health after ${STARTUP_TIMEOUT_SECONDS}s\"; exit 1; fi; sleep 0.25; done; echo \"[stack] executor is healthy, starting Agent Mockingbird\"; export PORT=3001; export AGENT_MOCKINGBIRD_PORT=3001; export AGENT_MOCKINGBIRD_DB_PATH=\"${STACK_DB_PATH}\"; export AGENT_MOCKINGBIRD_CONFIG_PATH=\"${STACK_CONFIG_PATH}\"; export AGENT_MOCKINGBIRD_OPENCODE_BASE_URL=\"http://127.0.0.1:3001\"; export AGENT_MOCKINGBIRD_OPENCODE_DIRECTORY=\"${OPENCODE_WORKSPACE_DIR}\"; export AGENT_MOCKINGBIRD_EXECUTOR_ENABLED=true; export AGENT_MOCKINGBIRD_EXECUTOR_BASE_URL=\"${EXECUTOR_BASE_URL}\"; export AGENT_MOCKINGBIRD_EXECUTOR_WORKSPACE_DIR=\"${EXECUTOR_WORKSPACE_DIR}\"; export AGENT_MOCKINGBIRD_EXECUTOR_DATA_DIR=\"${EXECUTOR_DATA_DIR}\"; export AGENT_MOCKINGBIRD_EXECUTOR_UI_MOUNT_PATH=\"/executor\"; export AGENT_MOCKINGBIRD_EXECUTOR_HEALTHCHECK_PATH=\"/executor\"; export AGENT_MOCKINGBIRD_EXECUTOR_MODE=\"embedded-patched\"; export AGENT_MOCKINGBIRD_MEMORY_API_BASE_URL=\"http://127.0.0.1:3001\"; export AGENT_MOCKINGBIRD_MEMORY_WORKSPACE_DIR=\"${OPENCODE_WORKSPACE_DIR}\"; export OPENCODE_CONFIG_DIR=\"${DEV_OPENCODE_CONFIG_DIR}\"; export OPENCODE_DISABLE_PROJECT_CONFIG=1; export OPENCODE_DISABLE_EXTERNAL_SKILLS=1; cd \"${ROOT_DIR}/apps/server\"; exec bun --hot src/index.ts'"
