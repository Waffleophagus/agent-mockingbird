...
- `agent-mockingbird.service` (dashboard/API plus embedded OpenCode runtime on `0.0.0.0:3001`)
...
- `agent-mockingbird.service` now hosts the embedded OpenCode server directly. The service listens on all interfaces at port `3001`, while the embedded runtime still targets the same app service through loopback (`http://127.0.0.1:3001` unless overridden).
