...
test("applyConfigPatch skips OpenCode semantic validation for directory-only changes", async () => {
  const tempRoot = mkdtempSync(path.join(tmpdir(), "agent-mockingbird-config-service-"));
  const configPath = path.join(tempRoot, "agent-mockingbird.config.json");
  const workspacePath = path.join(tempRoot, "workspace");
  const fixturePath = resolveExampleConfigPath();
  ...
  const blockingFetch = Object.assign(
    (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
      void input;
      void init;
      throw new Error("semantic validation should not call fetch for directory-only patches");
    },
    previousFetch,
  );
  ...
  try {
    ...
    expect(result.snapshot.config.runtime.opencode.directory).toBe(
      result.snapshot.config.workspace.pinnedDirectory,
    );
  } finally {
    globalThis.fetch = previousFetch;
    if (previousConfigPath === undefined) {
