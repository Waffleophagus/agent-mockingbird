...
export interface OpencodeRuntimeBackgroundMethods {
  ...
  ensureLocalSessionForBackgroundRun(run: BackgroundRunRecord, sessionInfo?: Session): string | null;
  ...
  applyBackgroundStatusBySessionId(opencodeSessionId: string, status: OpencodeSessionStatus): void;
...
export const opencodeRuntimeBackgroundMethods: OpencodeRuntimeBackgroundMethods = {
  ...
  backgroundRecordFingerprint(this: OpencodeRuntime, run: BackgroundRunRecord) {
    const childSessionId = getLocalSessionIdByRuntimeBinding(OPENCODE_RUNTIME_ID, run.childExternalSessionId) ?? "";
    return [
      run.status,
      run.error ?? "",
  ...
  ensureLocalSessionForBackgroundRun(this: OpencodeRuntime, run, sessionInfo) {
  ...
  mapOpencodeMessageContent(this: OpencodeRuntime, info, parts) {
    ...
    if (info.role === "assistant") {
      const failure = this.extractAssistantError(info as AssistantInfo, parts);
  ...
  async syncLocalSessionFromOpencode(this: OpencodeRuntime, input) {
    ...
    if (!input.force) {
      ...
      if (now - lastSyncedAt < BACKGROUND_MESSAGE_SYNC_MIN_INTERVAL_MS) return;
    }
    ...
    if (normalizedTitle && normalizedTitle !== existingSession.title) {
      ...
      if (updatedTitle) this.emit(createSessionStateUpdatedEvent(updatedTitle, "runtime"));
    }
    ...
    const synced = upsertSessionMessages({ sessionId: localSessionId, messages: imported, touchedAt: now });
    if (!synced) return;
    const entriesById = new Map(messages.map((entry) => [entry.info.id, entry] as const));
    const importedAssistants = imported.filter(
      (message): message is typeof message & { role: "assistant" } => message.role === "assistant",
    );
    for (const assistantMessage of importedAssistants) {
      ...
      const entryUpdatedAt = entryTime && "completed" in entryTime ? entryTime.completed : undefined;
      setMessageRenderSnapshot({
    ...
    for (const message of synced.inserted) {
      this.emit(createSessionMessageCreatedEvent({ sessionId: localSessionId, message }, "runtime"));
    }
  ...
  async syncBackgroundSessionMessages(this: OpencodeRuntime, run, force = false, messages) {
  ...
  ensureBackgroundRunForSessionInfo(this: OpencodeRuntime, sessionInfo, status = "created", knownParentSessionId) {
    ...
    const parentSessionId =
      ...
      getLocalSessionIdByRuntimeBinding(OPENCODE_RUNTIME_ID, parentExternalSessionId);
  ...
  async hydrateBackgroundRunFromSessionId(this: OpencodeRuntime, childExternalSessionId, status) {
  ...
  async announceBackgroundRunIfNeeded(this: OpencodeRuntime, runId) {
    ...
    try {
      ...
      this.emit(createSessionMessageCreatedEvent({ sessionId: appended.session.id, message: appended.message }, "runtime"));
  ...
  applyBackgroundStatusBySessionId(this: OpencodeRuntime, opencodeSessionId, status) {
  ...
  inFlightBackgroundChildRunCount(this: OpencodeRuntime, parentSessionId: string) {
